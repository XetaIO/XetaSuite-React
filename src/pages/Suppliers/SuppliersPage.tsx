...
export default function SuppliersPage() {
    ...
    const fetchSuppliers = useCallback(async (filters: SupplierFilters) => {
        ...
        setError(null);
        try {
            const response: PaginatedResponse<Supplier> = await supplierApi.getAll(filters);
    ...
    // Clear selection only when search changes (not when page changes)
    useEffect(() => {
    ...
    // Check if all suppliers on current page are selected
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            // await supplierApi.export(Array.from(selectedIds));
            //console.log('Exporting suppliers:', Array.from(selectedIds));
    ...
    return (
        <>
            <PageMeta
                title={`${t('suppliers.title')} | XetaSuite`}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
                ...
                <div className="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {/* Export button - visible only with permission and selection */}
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-b border-gray-200 dark:border-gray-800">
                                {/* Checkbox column - only if user can export */}
                                {canExport && (
                                    <TableCell
                                        isHeader
            ...
            <DeleteConfirmModal
                ...
                message={t('suppliers.deleteMessage', { name: selectedSupplier?.name })}
            />
        </>
    );
}
